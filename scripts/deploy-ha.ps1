...
function Quote-Sh([string]$Value) {
    if ($Value.Contains("'")) { throw "Remote path must not contain an apostrophe: $Value" }
    return "'$Value'"
}

function Get-ManagedFiles {
    $packageRoot = Join-Path $RepoRoot "packages"
    if (-not (Test-Path -LiteralPath $packageRoot -PathType Container)) { throw "Required package directory is missing: packages" }
...
function Get-ESPHomeNodeName {
    ...
    foreach ($line in $lines) {
        if ($line -match '^([ ]*)esphome:\s*(?:#.*)?$') {
            if ($inESPHomeBlock) { throw "Multiple top-level 'esphome:' blocks found in $ProjectFile." }
            $inESPHomeBlock = $true
        ...
        if ($line -match '^\s*name:\s*["'']?([a-zA-Z0-9_-]+)["'']?\s*(?:#.*)?$') { $foundNames += $Matches[1] }
    }
    if (-not $inESPHomeBlock) { throw "No top-level 'esphome:' block found in $ProjectFile." }
    if ($foundNames.Count -eq 0) { throw "No 'name:' entry found inside the 'esphome:' block in $ProjectFile." }
    if ($foundNames.Count -gt 1) { throw "Multiple 'name:' entries found inside the 'esphome:' block in $ProjectFile." }
    $nodeName = $foundNames[0]
    if ($nodeName -notmatch '^[a-z0-9][a-z0-9_-]*$') { throw "ESPHome node name '$nodeName' is not valid for automatic deployment filename generation." }
    return $nodeName
}

function Get-ProjectVersion {
    $versionPath = Join-Path $RepoRoot $VersionFile
    if (-not (Test-Path -LiteralPath $versionPath -PathType Leaf)) { throw "Central firmware version file is missing: $VersionFile" }
    $yaml = Get-Content -Raw -LiteralPath $versionPath
    $matches = [regex]::Matches($yaml, '(?m)^\s*firmware_version:\s*["'']?([0-9]+\.[0-9]+\.[0-9]+)["'']?\s*(?:#.*)?$')
    if ($matches.Count -eq 0) { throw "No valid firmware_version entry found in $VersionFile." }
    if ($matches.Count -gt 1) { throw "Multiple firmware_version entries found in $VersionFile." }
...
function Invoke-HaSsh([string]$Command) {
    $args = @("-i", $KeyPath, "-p", $Port, "-o", "BatchMode=yes", "-o", "ConnectTimeout=8", "-o", "StrictHostKeyChecking=accept-new", "-o", "MACs=$MacAlgorithm", "$HaUser@$HaHost", $Command)
    $output = & ssh @args
...
function Send-HaFile([string]$LocalPath, [string]$RemotePath) {
    $args = @("-i", $KeyPath, "-P", $Port, "-o", "BatchMode=yes", "-o", "ConnectTimeout=8", "-o", "StrictHostKeyChecking=accept-new", "-o", "MACs=$MacAlgorithm", $LocalPath, "$HaUser@${HaHost}:$RemotePath")
    & scp @args
    if ($LASTEXITCODE -ne 0) { throw "SCP upload failed for '$LocalPath' with exit code $LASTEXITCODE." }
}
...
function Get-ParentDirectories([string[]]$RelativePaths) {
    ...
    $sortProperties = @(@{ Expression = { ($_ -split '/').Count }; Ascending = $true }, @{ Expression = { $_ }; Ascending = $true })
...
function New-RemoteDirectories([string]$Root, [string[]]$RelativePaths) {
    ...
    foreach ($dir in (Get-ParentDirectories $RelativePaths)) { $commands += "mkdir -p $(Quote-Sh "$Root/$dir")" }
...
Assert-Command "scp"
$projectPath = Join-Path $RepoRoot $ProjectFile
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) { throw "Required deployment file is missing: $ProjectFile" }
$ManagedFiles = Get-ManagedFiles
foreach ($relativePath in $ManagedFiles) { if (-not (Test-Path -LiteralPath (Join-Path $RepoRoot $relativePath) -PathType Leaf)) { throw "Required deployment file is missing: $relativePath" } }
...
Write-Host ""
if ($gitInfo.Dirty) { Write-Warn "The Git working tree contains uncommitted changes; those local files will be deployed." }
if ($DryRun) {
    Write-Step "Dry run - no network connection and no remote changes"
    Write-Host "  $ProjectFile -> $RemoteDir/$RemoteProjectFile"
    foreach ($relativePath in $ManagedFiles) { Write-Host "  $relativePath -> $RemoteDir/$relativePath" }
    Write-Ok "Dry run complete"
    return
}
if (-not (Test-Path -LiteralPath $KeyPath -PathType Leaf)) { throw "SSH private key not found: $KeyPath`nRun .\scripts\setup-ha-ssh.ps1 first." }
...
try {
    ...
    foreach ($relativePath in $ManagedFiles) { Send-HaFile (Join-Path $RepoRoot $relativePath) "$StagingDir/$relativePath" }
    ...
    if ($remoteProjectHash -ne $projectHash) { throw "Hash mismatch after upload: $ProjectFile -> $RemoteProjectFile" }
    foreach ($relativePath in $ManagedFiles) {
        ...
        if ($remoteHash -ne $localHash) { throw "Hash mismatch after upload: $relativePath" }
    }
    ...
    if (-not $NoBackup) {
        ...
        foreach ($relativePath in $ManagedFiles) { $source = Quote-Sh "$RemoteDir/$relativePath"; $dest = Quote-Sh "$BackupDir/$relativePath"; $commands += "if [ -f $source ]; then cp -p $source $dest; fi" }
        Invoke-HaSsh ($commands -join "; ") | Out-Null
        Write-Ok "Backup created: $BackupDir"
    } else { Write-Warn "Backup disabled by -NoBackup" }
    ...
    foreach ($relativePath in $ManagedFiles) { $source = Quote-Sh "$StagingDir/$relativePath"; $dest = Quote-Sh "$RemoteDir/$relativePath"; $commands += "cp -p $source $dest" }
    Invoke-HaSsh ($commands -join "; ") | Out-Null
    ...
    if ($installedProjectHash -ne $projectHash) { throw "Installed file verification failed: $RemoteProjectFile" }
    foreach ($relativePath in $ManagedFiles) {
        $localHash = (Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $RepoRoot $relativePath)).Hash.ToLowerInvariant()
